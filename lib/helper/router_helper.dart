...
import 'package:flutter_restaurant/view/screens/checkout/widget/order_web_payment_guest.dart';
...
//*******Handlers*********
  ...
  static Handler _deshboardHandler1 = Handler(
  ...
  static Handler _deshboardHandler2 = Handler(handlerFunc: (context, Map<String, dynamic> params) {
  ...
  static Handler _deshboardHandler = Handler(handlerFunc: (context, Map<String, dynamic> params) {
    try{
      ...
      _routeHandler(context, DashboardScreen(pageIndex: 0,uid: params.length>0?(params['dashboard'][0]??"empty"):"empty",));
    }catch(error){
      _routeHandler(context, DashboardScreen(pageIndex: 0,uid: params.length>0?(params['dashboard'][0]??"empty"):"empty",));
    }
  ...
  static Handler _orderSuccessGuestHandler = Handler(
      handlerFunc: (context, Map<String, dynamic> params) {
        int _status = (params['status'][0] == 'success' || params['status'][0] == 'payment-success') ? 0 : params['status'][0] == 'fail' ? 1 : 2;
        return _routeHandler(context, OrderSuccessfulGuestScreen(orderID: params['id'][0], status: _status));
      }
  );
  ...
  static Handler _notFoundHandler1 = Handler(
...
//*******Route Define*********
  static void setupRouter() {
    ...
    router.define(Routes.DELIVERY_INFO_SCREEN, handler: _deshboardHandler, transitionType: TransitionType.fadeIn);
    ...
    //router.define(Routes.ORDER_SUCCESS_GUEST_SCREEN +  '/:id/:status', handler: _orderSuccessGuestHandler, transitionType: TransitionType.fadeIn);
    router.define(Routes.ORDER_WEB_PAYMENT + '/:status?:token', handler: _orderWebPaymentHandler, transitionType: TransitionType.fadeIn);
    router.define(Routes.ORDER_SUCCESS_GUEST_SCREEN + '/:status?:token', handler: _orderWebPaymentGuestHandler, transitionType: TransitionType.fadeIn);
