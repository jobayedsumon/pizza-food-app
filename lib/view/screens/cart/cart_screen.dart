                                    ...
                                    child: Row(
                                      ...
                                      children: [
                                        if(ResponsiveHelper.isDesktop(context)) Expanded(child: Padding(
                                          ...
                                          child: CartListWidget(cart: cart,addOns: _addOnsList, availableList: _availableList,allergies: _allergiesList,),
                                        )),
                                        ...
                                        padding: ResponsiveHelper.isDesktop(context) ? EdgeInsets.symmetric(horizontal: Dimensions.PADDING_SIZE_LARGE,vertical: Dimensions.PADDING_SIZE_LARGE) : EdgeInsets.all(0),
                                            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                              ...
                                              // Product
                                              if(!ResponsiveHelper.isDesktop(context)) CartListWidget(cart: cart,addOns: _addOnsList, availableList: _availableList,allergies: _allergiesList,),
...
class CartListWidget extends StatelessWidget {
  ...
  final List<List<Allergies>> allergies;
  final List<bool> availableList;
  const CartListWidget({Key key, @required this.cart, @required this.addOns, @required this.availableList, @required this.allergies}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        return CartProductWidget(cart: cart.cartList[index], cartIndex: index, addOns: addOns[index], isAvailable: availableList[index],allergies: allergies[index],);
      },
    );
  }
}

class CartListMenuWidget extends StatelessWidget {
  final CartProvider cart;
  final List<List<Allergies>> allergies;
  final List<List<AddOns>> addOns;
  final List<bool> availableList;
  const CartListMenuWidget({Key key, @required this.cart, @required this.addOns, @required this.availableList, @required this.allergies}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return ListView.builder(
      ...
      itemCount: cart.cartList.length,
      itemBuilder: (context, index) {
        return CartProductMenuWidget(cart: cart.cartList[index], cartIndex: index, addOns: addOns[index], isAvailable: availableList[index],allergies: allergies[index]);
      },
    );
  }
}
