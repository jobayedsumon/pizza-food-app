...
class CheckoutWithoutLoginScreen extends StatefulWidget {
  final double amount;
  final String orderType;
  final List<CartModel> cartList;
  final bool fromCart;
  final String couponCode;
  CheckoutWithoutLoginScreen({ @required this.amount, @required this.orderType, @required this.fromCart,
    @required this.cartList, @required this.couponCode});

  @override
  _CheckoutWithoutLoginScreenState createState() => _CheckoutWithoutLoginScreenState();
}

class _CheckoutWithoutLoginScreenState extends State<CheckoutWithoutLoginScreen> {
              ...
              if(widget.amount < Provider.of<SplashProvider>(context, listen: false).configModel.minimumOrderValue) {
                ...
                for (int index = 0; index < _cartList.length; index++) {
                  ...
                  cart.allergIds.forEach((allergies) {
                    _allergiesIdList.add(allergies.id);
                  });
                ...
                if(_isCashOnDeliveryActive && Provider.of<OrderProvider>(context, listen: false).paymentMethodIndex == 0) {
                  ...
                  String _url = "customer_id=${Provider.of<ProfileProvider>(context, listen: false).userInfoModel.id}"
                      "&&callback=${AppConstants.BASE_URL}${Routes.ORDER_SUCCESS_SCREEN}&&order_amount=${(widget.amount).toStringAsFixed(2)}";

                  String _webUrl = "customer_id=${Provider.of<ProfileProvider>(context, listen: false).userInfoModel.id}"
                      "&&callback=$protocol//$hostname:$port${Routes.ORDER_WEB_PAYMENT}&&order_amount=${(widget.amount).toStringAsFixed(2)}&&status=";
