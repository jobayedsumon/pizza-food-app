...
class OrderProvider extends ChangeNotifier {
  ...
  Future<void> placeOrderGuest(PlaceOrderGuestBody placeOrderBody, Function callback) async {
    _isLoading = true;
    notifyListeners();

    ApiResponse apiResponse = await orderRepo.placeOrderGuest(placeOrderBody);
    _isLoading = false;
    if (apiResponse.response != null && apiResponse.response.statusCode == 200) {
      String message = apiResponse.response.data['message'];
      String orderID = apiResponse.response.data['order_id'].toString();
  ...
  Future<void> initializeTimeSlot(BuildContext context) async {
   ...
   int _duration = Provider.of<SplashProvider>(context, listen: false).configModel.scheduleOrderSlotDuration;
    ...
    validateSlot(_allTimeSlots, _selectDateSlot, notify: false);
  }
